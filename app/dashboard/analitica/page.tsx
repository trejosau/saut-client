...
export default async function AnalyticsDashboardPage() {
  ...
  return (
    <main className="w-full px-4 py-10 sm:px-8 lg:px-14">
      <section className="rounded-[20px] border border-(--border) bg-[rgba(255,255,255,.44)] p-5 sm:p-6">
        <p className="text-[10px] font-black uppercase tracking-[0.14em] text-[rgba(8,10,13,.64)]">
          Bloque 12 | Analitica and Mapa
        </p>
        <h1 className="mt-1 text-[28px] sm:text-[36px] font-black uppercase tracking-[0.04em] text-(--text)">
          KPI de Operacion
        </h1>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <article className="rounded-[14px] border border-(--border) bg-white/80 p-3">
            <p className="text-[10px] font-black uppercase tracking-[0.1em] text-(--text)">Pedidos pagados</p>
            <p className="mt-1 text-[22px] font-black text-(--text)">{kpis.sales.paid_orders}</p>
          </article>
          <article className="rounded-[14px] border border-(--border) bg-white/80 p-3">
            <p className="text-[10px] font-black uppercase tracking-[0.1em] text-(--text)">Revenue MXN</p>
            <p className="mt-1 text-[22px] font-black text-(--text)">${money(kpis.sales.revenue_mxn)}</p>
          </article>
          <article className="rounded-[14px] border border-(--border) bg-white/80 p-3">
            <p className="text-[10px] font-black uppercase tracking-[0.1em] text-(--text)">Merma unidades</p>
            <p className="mt-1 text-[22px] font-black text-(--text)">{kpis.merma.units}</p>
          </article>
          <article className="rounded-[14px] border border-(--border) bg-white/80 p-3">
            <p className="text-[10px] font-black uppercase tracking-[0.1em] text-(--text)">Incidencias abiertas</p>
            <p className="mt-1 text-[22px] font-black text-(--text)">{kpis.incidents.open_cases}</p>
          </article>
        </div>

        <div className="mt-3 grid gap-3 sm:grid-cols-3">
          <article className="rounded-[12px] border border-(--border) bg-white/75 p-3 text-[11px]">
