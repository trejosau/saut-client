...
export function LandingFooter() {
    return (
        <footer className="bg-(--saut-black) text-white">
                ...
                <div className="grid gap-10 border-b border-white/15 pb-12 lg:grid-cols-[1.1fr_.9fr] lg:items-end">
                    <div>
                        ...
                        <p className="mt-4 max-w-[52ch] text-sm leading-6 text-white/66">
                            Streetwear y prendas personalizadas producidas con atención al detalle.
                        </p>
                    </div>

                    <section aria-labelledby="newsletter-title" className="lg:justify-self-end lg:text-right">
                        <p className="text-xs font-extrabold uppercase text-(--saut-yellow)">
                            Suscríbete
                        </p>
                ...
                <div className="grid gap-10 py-12 sm:grid-cols-2 lg:grid-cols-4">
                    <section>
                        ...
                        <nav aria-label="Compra" className="mt-4 grid gap-3">
                            {shopLinks.map((link) => (
                                <Link key={link.href} href={link.href} className="w-fit text-sm font-bold hover:text-(--saut-yellow)">
                                    {link.label}
                                </Link>
                            ))}
                        </nav>
                    ...
                    <section>
                        ...
                        <div className="mt-4 grid gap-4 text-sm text-white/72">
                            <p className="flex items-start gap-3">
                                <PackageCheck className="mt-0.5 shrink-0 text-(--saut-yellow)" size={19} />
                                Envíos nacionales con seguimiento.
                            </p>
                            <p className="flex items-start gap-3">
                                <Shirt className="mt-0.5 shrink-0 text-(--saut-yellow)" size={19} />
                                Cambios antes de entrar a producción.
                            </p>
                        </div>
                    </section>
                    ...
                    <section>
                        ...
                        <p className="mt-4 flex items-center gap-3 text-sm font-bold text-white/72">
                            <CreditCard className="text-(--saut-yellow)" size={20} />
                            Visa · Mastercard · Amex
                        </p>
                        <p className="mt-2 text-sm text-white/56">Stripe · Apple Pay · Google Pay</p>
                    </section>
                </div>
